...
namespace Foo
{
    partial class FooBar
    {
        ...
        [Fact]
        public void GeneratedNamedOneOf()
        {
            // lang=cs
            const string input = """
                                 using OneOf;
                                 namespace Foo { 
                                     [GenerateNamedOneOf]
                                     partial class Foo : OneOfBase<string, int> { }
                                 }
                                 """;
            ...
            const string expectedCode = """
                                        ...
                                        partial class Foo
                                        {
                                            public Foo(OneOf.OneOf<string, int> _) : base(_) { }
                                        
                                            public static implicit operator Foo(string _) => new Foo(_);
                                            public static explicit operator string(Foo _) => _.AsT0;
                                        
                                            public static implicit operator Foo(int _) => new Foo(_);
                                            public static explicit operator int(Foo _) => _.AsT1;
                                        
                                            // T0: string
                                            public bool IsString => IsT0;
                                            public string AsString => AsT0; 
                                            public bool TryPickString(out string value, out int remainder)
                                                => TryPickT0(out value, out remainder);
                                        
                                            // T1: int
                                            public bool IsInt => IsT1;
                                            public int AsInt => AsT1; 
                                            public bool TryPickInt(out int value, out string remainder)
                                                => TryPickT1(out value, out remainder);
                                        }
                                        
                                        """;
            
            AssertCorrectSourceCodeIsGeneratedWithNoDiagnostics(input, expectedCode, "Foo_Foo.g.cs", generator: new NamedOneOfGenerator());
        }

        [Fact]
        public void Does_Not_Generate_NamedOneOf_With_Duplicate_Types()
        {
            // lang=cs
            const string input = """
                                 using OneOf;
                                 namespace Foo { 
                                     [GenerateNamedOneOf]
                                     partial class Foo : OneOfBase<string, string> { }
                                 }
                                 """;
            
            AssertDiagnosticErrorIsReturned(input, GeneratorDiagnosticDescriptors.DuplicateTypesAreNotAllowed.Id, new NamedOneOfGenerator());
        }

        private static void AssertCorrectSourceCodeIsGeneratedWithNoDiagnostics(string inputSource, string expectedCode, string generatedFileName, int expectedCompilationFileCount = 3, IIncrementalGenerator? generator = null)
        {
            var parsedAttribute = CSharpSyntaxTree.ParseText(expectedCode);

            var inputCompilation = CreateCompilation(inputSource);

            GeneratorDriver driver = CSharpGeneratorDriver.Create(generator ?? new OneOfGenerator());
        ...
        private static void AssertDiagnosticErrorIsReturned(string inputSource, string diagnosticId, IIncrementalGenerator? generator = null)
        {
            var inputCompilation = CreateCompilation(inputSource);
